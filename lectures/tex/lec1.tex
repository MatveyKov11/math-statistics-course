...
$F_n(x)$ - эмпирическая функция распределения \par 
...
$\mathbb{P}(\{ X_{(k)} \in x, $[$x + dx$)$) ) = $ *$k-1$ элементов $< x$, один элемент в интервале (всего элементов $n$)* $= n\mathbb{P}(\{ X_1 \in x, x+dx; \text{ровно } k-1 \text{ элементов выборки } < x\} ) = P($[$x, x+dx$)$)C_{n-1}^{k-1}(F(x))^{k-1}(1 - F(x))^{n-k} + o(dx)$, устремим $dx \rightarrow 0$ и получим плотность $k$-ой порядковой статистики \par
...
2) По ЗБЧ (закону больших чисел) $F_n(x) \rightarrow^{\mathbb{P}} F(x)$ \par
т. е. $\forall \epsilon > 0\ \mathbb{P}(\{|F_n(x) - F(x)| > \epsilon\}) \rightarrow_{n \rightarrow \infty} 0$ \par 
