...
$\sqrt{n}(\widehat{\theta}_n - \theta) = \frac{U(\theta)}{\sqrt{n}}(-\frac{U'(\theta)}{n} - \frac{\epsilon_n}{n})^{-1}$, множитель перед скобкой сходится по распределению к $\xi \sim N(0, \frac{1}{i_1(\theta)})$, первая дробь в скобке стремится по вероятности к единице \par 
