...
Эффективное оценивание возможно для $\theta$ \par 
...
$\int\frac{1}{a_1(\theta)}(T^*(x) - \tau(\theta))d\theta = \ln f(x, \theta)$ \par 
$A(\theta) = \int\frac{d\theta}{a_1(\theta)}$, $B(x) = T^*(x)$, $C(\theta) = -\int\frac{\tau(\theta)}{a_1(\theta)}$ \par 
Возводим и в итоге: \par 
...
В критерии Бхаттачария кроме выписанной дроби используются ещё дроби вида $\frac{L_{\theta}^{(2)}(X, \theta)}{L(X, \theta)}, \ldots, \frac{L_{\theta}^{(n)}(X, \theta)}{L(X, \theta)}$ \par 
...
Обобщение неравенства Рао-Крамера \par 
\end{document}