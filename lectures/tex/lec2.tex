...
Проверка для $k=2$: $(\overline{X})^2 - 2\overline{X}\cdot\overline{X} + \overline{X^2} = \overline{X^2} - (\overline{X})^2 = S_n^2$ \par 
Свойства оценок: \par 
...
$E(\overline{X})^2 = E(\frac{1}{n}\sum\limits_{i = 1}^nX_i)(\frac{1}{n}\sum\limits_{j = 1}^nX_j) = \frac{1}{n^2}\sum\limits_{i = 1}^n\sum\limits_{j = 1}^nEX_iX_j = \frac{1}{n^2}(\sum\limits_{i=j} EX_i^2 + \sum\limits_{i \neq j} EX_iX_j) = \frac{1}{n^2}(nEX_i^2 + n(n-1)EX_iX_j) = \frac{1}{n}\alpha_2 + \frac{n-1}{n}\alpha_1^2 = \frac{1}{n}(\alpha_2 - \alpha_1^2) + \alpha_1^2 = \frac{\sigma^2}{n} + \alpha_1^2$ \par 
...
$\eta_i(n) \rightarrow^{\mathbb{P}} c_i$, то есть $\forall i \forall \delta_i > 0\ \mathbb{P}(\lbrace | \eta_i(n) - c_i| > \delta_i \rbrace) \rightarrow_{n \rightarrow \infty} 0$ \par 
...
$\widetilde{S_n^2} = \frac{n}{n-1} S_n^2 = \frac{1}{n-1} \sum\limits_{i = 1}^n(X_i - \overline{X})^2$ - исправленная выборочная дисперсия, несмещённая оценка для дисперсии \par 
...
Тогда $\forall c_1, c_2\ \widetilde{T} = c_1T_1^* + c_2T_2^*$ - оптимальная оценка для $\tau = c_1\tau_1(\theta) + c_2\tau_2(\theta)$ \par 
Доказательство \par 
$\widetilde{T}$ - статистика, несмещённая для $\tau$ \par 
Пусть $T$ - несмещённая оценка для $\tau(\theta)$ \par 
$D_{\theta}T = D_{\theta}(T - \widetilde{T} + \widetilde{T}) = D_{\theta}(T - \widetilde{T}) + D_{\theta}\widetilde{T} + 2cov(T - \widetilde{T}, \widetilde{T}) \geq D_{\theta}\widetilde{T} + 2cov(T - \widetilde{T}, \widetilde{T})$ \par
Надо доказать, что $cov(T - \widetilde{T}, \widetilde{T}) = 0$ \par 
Заметим, что $E_{\theta}(T - \widetilde{T}) = 0$ \par 
$\widehat{T_1} = T_1^* + \lambda(T - \widetilde{T})$ - несмещённая оценка $\tau_1(\theta)$ \par 
$D_{\theta}\widehat{T_1} \geq D_{\theta}T_1^*\ \forall \lambda \in \mathbb{R}^1$ \par 
$D_{\theta}\widehat{T_1} = D_{\theta}T_1^* + \lambda^2D_{\theta}(T - \widetilde{T}) + 2\lambda cov(T_1^*, T - \widetilde{T}) \geq D_{\theta}T_1^*$ \par
$\lambda^2D_{\theta}(T - \widetilde{T}) + 2\lambda cov(T_1^*, T - \widetilde{T}) \geq 0$ - имеет 2 корня и всегда неотрицателен $\Rightarrow$ корни совпадают $\Rightarrow\ cov(T_1^*, T - \widetilde{T}) = 0$ \par 
$cov(\widetilde{T}, T - \widetilde{T}) = c_1 cov(T_1^*, T - \widetilde{T}) + c_2 cov(T_2^*, T - \widetilde{T}) = 0$ - QED \par 
\end{document}